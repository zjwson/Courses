...
void bdaemon_run(){
     ...
     * 执行setid后，child将重新获得一个新的会话id
     * 这是parent退出之后，将不会影响到child了
     * */
    setsid();
    int fd;
...
bool create_server_listener(const char*  ip, short port){
    ...
    e.data.fd = g_listenfd;//主线程往epoll内核注册socket上的读就绪事件
...
void release_client(int clientfd){
    ...
    //关闭连接，并将clientfd从g_epollfd中删除
    if(::epoll_ctl(g_epollfd, EPOLL_CTL_DEL, clientfd, NULL) == -1)
        std::cout << "release client socket failed as call epoll_call failed" << std::endl;
    ::close(clientfd);//断开连接
}
...
void* accept_thread_func(void* arg){
    ...
    while(!g_bStop){
        ::pthread_mutex_lock(&g_acceptmutex);//先获得与条件变量相关的mutex
        ::pthread_cond_wait(&g_acceptcond, &g_acceptmutex);//被唤醒后说明有新连接到来
        //::phread_mutex_lock(&g_acceptmutex);
        ...
        if(::fcntl(newfd, F_SETFL, newflag) == -1){
            std::cout << "fcnfl error, oldflag = " << oldflag
        ...
        struct epoll_event e;
        memset(&e, 0, sizeof(e));
        e.events = EPOLLIN | EPOLLRDHUP | EPOLLET;//对新连接采用边沿模式
        e.data.fd = newfd;
...
void* worker_thread_func(void* arg){
    while(!g_bStop){
        ...
        while(true){
            ...
            strclientmsg.erase(0, nSent);//把发送出去的部分拿掉，循环发送剩余部分
    ...
    while(!g_bStop){
        ...
        for(int i = 0; i < m; ++i){
            ...
            //通知普通工作线程接收数据
                ...
                pthread_mutex_lock(&g_clientmutex);
                g_listClients.push_back(ev[i].data.fd);//把发生事件的fd放入列表中
                pthread_mutex_unlock(&g_clientmutex);//解锁，让工作线程获得锁
                pthread_cond_signal(&g_cond);//通知工作线程处理事件
